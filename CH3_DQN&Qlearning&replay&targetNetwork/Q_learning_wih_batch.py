    ...
    for i in range(epochs):
        ...
        while(status == 1) :
            ...
            if len(mem_list) >= batch_size :
                ...
                batch_state1 = torch.cat([ state1 for (state1 , action_int , reward , state2 , done) in minibatch])
                batch_action_int = torch.Tensor([ action_int for (state1 , action_int , reward , state2 , done) in minibatch])
                batch_reward = torch.Tensor([ reward for (state1 , action_int , reward , state2 , done) in minibatch])
                batch_state2 = torch.cat([ state2 for (state1 , action_int , reward , state2 , done) in minibatch])
                batch_done = torch.Tensor([ done for (state1 , action_int , reward , state2 , done) in minibatch])
