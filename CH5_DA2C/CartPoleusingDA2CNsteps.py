...
def runEpisode(worker_env,worker_model):
    ...
    while(j < n_steps and done == False):
        ...
        # calculate graph generate
        # if j == 1:  # 只生成第一集第一个状态的图
        #    make_dot(policy.mean(), params=dict(worker_model.named_parameters()),show_attrs=False, show_saved=False).render(f"policy_graph", format="png")
        #    make_dot(value.mean(), params=dict(worker_model.named_parameters()), show_attrs=False, show_saved=False).render(f"value_graph", format="png")
